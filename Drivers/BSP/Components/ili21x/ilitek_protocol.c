...
//  }
    ...
    for (i = 0; i < report_max_point; i++) {
        ...
        if (ilitek_data->tp[i].status) {
            ...
            // Resolution
			if (ilitek_data->tp_max_x != 1024) {
				LOG_ERR("exception tp_max_x != 1024 is %d set tp_max_x = 1024\n", ilitek_data->tp_max_x);
				ilitek_data->tp[i].x = (ilitek_data->tp[i].x * 1024) / ilitek_data->tp_max_x;
			}
			if (ilitek_data->tp_max_y != 600) {
				LOG_ERR("exception tp_max_y != 600 is %d set tp_max_y = 600\n", ilitek_data->tp_max_y);
				ilitek_data->tp[i].y = (ilitek_data->tp[i].y * 600) / ilitek_data->tp_max_y;
			}
